...
<div class="container">
  ...
  <div class="row">
    <div class="col-sm-6">
      <form id="course" action="{{ route('manage.update', $course->id) }}" method="post">
        {{ csrf_field() }}
        {{ method_field('PUT') }}
        <h4 class="m-b-20">Course details:</h4>
        <input type="hidden" id='lectii' value="{{ $course->lessons }}">
        ...
        <div class="form-group">
          ...
          <textarea name="course_description" class="form-control" id="course_description">{{ $course->description }}</textarea>
        </div>
        <div class="checkbox pull-right">
          <label>
            <input type="checkbox" name="course_published" {{ ($course->published == 1)? "checked" : "" }} >
            Published?
          </label>
        </div>
        <hr style="clear:both;">
  ...
  <hr>
  <button class="btn btn-primary pull-right" id="update_course"><i class="fa fa-book m-r-10"></i> Update Course</button>
</div>
...
$( function() {
  ...
  let lectii = JSON.parse($( "#lectii" ).val());
  populate_lessons(lectii);
  ...
  function populate_lessons(lectii)
  {
    for (var i = 0; i < lectii.length; i++) {
      let lectie = lectii[i];
      let lectie_title = lectie.title;
      let lectie_description = lectie.description;
      let movieId = lectie.video_id;
      let img_url = "https://img.youtube.com/vi/" + movieId + "/0.jpg";

      $( "#sortable" ).append(`
        <li class="list-group-item alert" data-lesson_title="${lectie_title}" data-lesson_description="${lectie_description}" data-movieid="${movieId}" style="margin-bottom:10px;">
          <button type="button" class="close" data-dismiss="alert" aria-label="Close">
          <div aria-hidden="true">&times;</div></button>
          <div class="row">
          <div class="col-sm-6">
          <div class="m-t-25"><strong>Title:</strong> ${lectie_title}</div>
          <div class="m-t-5"><strong>Description: </strong>${lectie_description}</div>
          </div>
          <div class="col-sm-6">
          <div><img src="${img_url}" style="width:150px; margin-left:40px;"></div>
          </div>
